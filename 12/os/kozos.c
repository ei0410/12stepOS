...
typedef struct _kz_msgbox {
	...
	long dummy[1]; // make dummy member because you should 2^n size of structure
} kz_msgbox;
...
static int getcurrent(void) // pull current thread from ready queue
{
	...
	// if status is not ready, do nothing.
	...
	current->flags &= ~KZ_THREAD_FLAG_READY; // fall ready bit
...
static int putcurrent(void)
{
	...
	current->flags |= KZ_THREAD_FLAG_READY; // wake up ready bit
...
static int thread_chpri(int priority) 
{
	...
	// change priority and connect new ready queue
	if (priority >= 0) 
...
static void sendmsg(kz_msgbox *mboxp, kz_thread *thp, int size, char *p)
{
	...
	// connect message at message box tail
	if (mboxp->tail) {
		mboxp->tail->next = mp;
	} else {
		mboxp->head = mp;
...
static void recvmsg(kz_msgbox *mboxp)
{
	...
	// set return value to thread received message
	...
	// set NULL because no thread waiting receiving
	mboxp->receiver = NULL;
...
static int thread_send(kz_msgbox_id_t id, int size, char *p)
{
	...
	// if waiting receiveing thread exist, do receiving process.
	if (mboxp->receiver) {
...
{
	// warn change current while run syscall
	switch (type) {
		...
		// don't write return value because TCB delete
			thread_exit();
			break;
...
static void schedule(void)
{
	...
	// sort ready queue array, search for enable thread
...
static void softerr_intr(void)
{
	puts(current->name);
	puts(" DOWN.\n");
	getcurrent(); // get out from readyque
	thread_exit();
}

static void thread_intr(softvec_type_t type, unsigned long sp)
{
	current->context.sp = sp; // save context of current thread to TCB
